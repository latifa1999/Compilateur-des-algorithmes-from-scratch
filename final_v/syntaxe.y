...
program : {fprintf(f,"#include<stdio.h>\n#include<stdlib.h>\n#include<string.h>\n"); } listFonction main
		| main 
...
listFonction : FUNCTION type  DEUX_POINTS IDENTIF {strcpy(fname,identif); }
  PARTH_O listArg PARTH_F {funcTest(fname,&fhead,pn,args,tokens);pn=0} DEB_FCT declarationF listinstF return FINFCT 

;
return : RETOURNE expressionArithmeticF  POINT_VIRGULE
;
...
declarationF : declarationF VAR type  DEUX_POINTS listidentifs   POINT_VIRGULE 
			| VAR type DEUX_POINTS listidentifs   POINT_VIRGULE
;
...
switchF : SELON PARTH_O IDENTIF{fprintf(f,"%s",identif);type=-1;strcpy(autre,identif);} {if(type_return==-1){isUsableF(heads,identif,0);giveValuef(heads,identif);}else{isUsableF(heads,identif,1);}} PARTH_F ACC_O listdesCasF ACC_F {printf("switch correct\n");} 
;

listdesCasF : listdesCasF lesCasF
			| lesCasF
;
lesCasF : CAS valeur DEUX_POINTS listinstF BREAK POINT_VIRGULE
...
pourF : POUR IDENTIF ALLANT exprF TO exprF  FAIRE listinstF FINPOUR 
;
...
lectureF : LIRE PARTH_O appel_varsF PARTH_F POINT_VIRGULE 
;

appel_varsF : appel_varsF VIRGULE IDENTIF { isUsableF(heads,identif,0); fprintf(f,"scanf(\" ");}
		   | IDENTIF { isUsableF(heads,identif,0); fprintf(f,"scanf(\" ");}
;

affichageF : AFFICHER PARTH_O parametresF PARTH_F POINT_VIRGULE 
;
...
assignationAddF : IDENTIF {fprintf(f,"%s",identif);
        		  ...
        		  {if(type_return==-1){
        		  	...
        		  	isUsableF(heads,identif,1);}
        		  }ASS_ADD listinstF POINT_VIRGULE 
;
assignationSousF: IDENTIF {fprintf(f,"%s",identif);
        		  ...
        		  {if(type_return==-1){
        		  	...
        		  	isUsableF(heads,identif,1);}
        		  } ASS_SOUS listinstF POINT_VIRGULE 
;
assignationMultF: IDENTIF {fprintf(f,"%s",identif);
        		  ...
        		  {if(type_return==-1){
        		  	...
        		  	isUsableF(heads,identif,1);}
        		  } ASS_MULT listinstF POINT_VIRGULE 
;
assignationDivF : IDENTIF {fprintf(f,"%s",identif);
        		  ...
        		  {if(type_return==-1){
        		  	...
        		  	isUsableF(heads,identif,1);}
        		  } ASS_DIV listinstF POINT_VIRGULE 
;
...
incrementationF : IDENTIF {fprintf(f,"%s",identif);
			   ...
			   | INC IDENTIF{fprintf(f,"%s",identif);
        		   type=-1;strcpy(autre,identif);}
        		  {if(type_return==-1){
        		  		isUsableF(heads,identif,0);
        		  		giveValuef(heads,identif);
...
lecture : LIRE PARTH_O {fprintf(f,"\tscanf(\"");} appel_vars PARTH_F POINT_VIRGULE 
;

appel_vars : appel_vars VIRGULE IDENTIF { isUsable(head,identif,0);}
		   | IDENTIF { isUsable(head,identif,0);
...
appelFct : IDENTIF { if(type_return==-1){
					  		isUsable(head,identif,0);
					  	}
					  else{
					  isUsable(head,identif,1);
					  	}
				} EGAL IDENTIF  PARTH_O {strcpy(fname,identif);isUsablef(fhead,fname);} Parametre {isParaEqual(fhead,fname,pn);pn=0;} PARTH_F POINT_VIRGULE
; 
